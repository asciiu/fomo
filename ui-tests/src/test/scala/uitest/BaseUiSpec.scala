...
import com.flowy.fomoApi.services.{DependencyWiring, HttpService}
...
import com.softwaremill.bootzooka.DependencyWiring
import org.openqa.selenium.firefox.FirefoxDriver
import org.openqa.selenium.support.PageFactory
import org.scalatest.concurrent.ScalaFutures
import org.scalatest.{BeforeAndAfter, BeforeAndAfterAll, FunSuite, Matchers}
import services.DependencyWiring
...
class BaseUiSpec extends FunSuite with Matchers with BeforeAndAfterAll with BeforeAndAfter with ScalaFutures {
  ...
  override def beforeAll() {
    val (startFuture, _beans) = new HttpService().start()
